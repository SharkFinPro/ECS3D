...
namespace {
  ...
  SceneEditResult apply(const Scene& scene, const nlohmann::json& edit,
...
TEST(SceneEdit, AppliesAnAddObjectEdit)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildAddObject("Added")), SceneEditResult::applied);
...
TEST(SceneEdit, ReportsAnEditWithNoOpAsMalformed)
{
  ...
  EXPECT_EQ(apply(scene, nlohmann::json::object()), SceneEditResult::malformedEdit);
}
...
TEST(SceneEdit, ReportsAnOpNothingHandlesAsMalformed)
{
  ...
  EXPECT_EQ(apply(scene, nlohmann::json{ { "op", "teleportEverything" },
...
TEST(SceneEdit, ReportsAnOpMissingTheFieldItNeedsAsMalformed)
{
  ...
  EXPECT_EQ(apply(scene, nlohmann::json{ { "op", "renameObject" },
                                         { "object", uuids::to_string(scene.object->getUUID()) } }),
            SceneEditResult::malformedEdit);
}
...
TEST(SceneEdit, ReportsAnObjectUuidThatDoesNotParseAsMalformed)
{
  ...
  EXPECT_EQ(apply(scene, nlohmann::json{ { "op", "removeObject" }, { "object", "not-a-uuid" } }),
            SceneEditResult::malformedEdit);
}
...
TEST(SceneEdit, ReportsAnObjectTheSceneDoesNotHaveAsUnknown)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildRemoveObject(someOtherUUID())), SceneEditResult::unknownObject);
...
TEST(SceneEdit, ReportsAComponentTypeThatDoesNotExistAsUnknown)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildAddComponent(scene.object->getUUID(), "Nope")),
...
TEST(SceneEdit, ReportsRemovingAComponentTheObjectDoesNotCarryAsUnknown)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildRemoveComponent(scene.object->getUUID(), collider)),
            SceneEditResult::unknownComponent);
}
...
TEST(SceneEdit, ReportsAReparentThatWouldCycleAsRejected)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildReparentObject(scene.object->getUUID(), &parentUUID)),
...
TEST(SceneEdit, ReportsAReparentOntoTheCurrentParentAsRejected)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildReparentObject(child->getUUID(), &parentUUID)),
            SceneEditResult::rejected);
}
...
TEST(SceneEdit, ReportsAPrefabWithNoRegistryToResolveItAsUnknown)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildInstantiatePrefab(someOtherUUID())),
...
TEST(SceneEdit, ReportsAPrefabUuidTheRegistryDoesNotHaveAsUnknown)
{
  ...
  EXPECT_EQ(apply(scene, replication::buildInstantiatePrefab(someOtherUUID()), &assetRegistry),
            SceneEditResult::unknownAsset);
}
