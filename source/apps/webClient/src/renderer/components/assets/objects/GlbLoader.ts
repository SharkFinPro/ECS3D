...
export async function loadGlb(url: string): Promise<GltfMesh> {
...
function parseContainer(data: ArrayBuffer, url: string): { json: Gltf; bin: Uint8Array } {
...
function buildMesh(json: Gltf, bin: Uint8Array, url: string): GltfMesh {
...
function appendPrimitive(
  ...
  // glTF: when a node's global transform has a negative determinant (any mirroring), the winding of
  // its triangles must be reversed, or the mesh renders inside-out. Invisible while nothing culled;
  // load-bearing now that the object/shadow pipelines cull back faces.
  const mirrored = determinant3(worldMatrix) < 0;
  ...
  if (primitive.indices !== undefined) {
    ...
    for (let i = 0; i + 2 < primitiveIndices.length; i += 3) {
      triangle(
        baseVertex + primitiveIndices[i],
        baseVertex + primitiveIndices[i + 1],
        baseVertex + primitiveIndices[i + 2],
      );
    }
  } else {
    for (let i = 0; i + 2 < vertexCount; i += 3) {
      triangle(baseVertex + i, baseVertex + i + 1, baseVertex + i + 2);
    }
  }
...
function readAccessor(json: Gltf, bin: Uint8Array, index: number, url: string): Float64Array {
...
function computeNormals(vertices: number[], indices: number[], baseVertex: number): void {
...
function localMatrix(node: GltfNode): Float64Array {
...
// Determinant of the upper-left 3x3 - the rotation/scale part, whose sign says whether the transform
// mirrors. The translation column cannot affect handedness, so the full 4x4 is not needed.
...
function invert(m: Float64Array): Float64Array {
