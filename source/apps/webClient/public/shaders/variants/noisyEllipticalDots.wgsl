// Port of StandardObject.vert + NoisyEllipticalDots.frag: elliptical dots whose distance
// field is warped by a 3D value-noise sample, then Phong over point lights.
// Groups 0/1/2 match standard.wgsl; group 3 = params + noise volume.
...
fn fs_main(in : VertexOut) -> @location(0) vec4<f32> {
  let nv = textureSample(noiseTexture, noiseSampler, params.noiseFrequency * in.worldPosition);
  var n = nv.r + nv.g + nv.b + nv.a;
  n = n - 2.0;
  n = n * params.noiseAmplitude;
  ...
  let oldDist = sqrt(ds * ds + dt * dt);
  let scale = (oldDist + n) / oldDist;
