...
// writes them here, and the InputUtils bindings read them. This replaces the old InputUtils reading
// the local GLFW window directly. Set + read happen on the server's main loop thread (the inputState
// message is drained and the scripts run on the same thread), so no locking is needed.
class InputState {
public:
