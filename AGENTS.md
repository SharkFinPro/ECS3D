...
| `source/libs/sim/` | `ECS3DSim` — `PhysicsSystem` (integration, forces, response; linear velocity is displacement per tick while angular velocity is a world-space axis in degrees per second, so convert angular velocity to radians per tick before combining the two; it turns the orientation by composing a rotation onto it rather than by adding to the Euler angles, and an off-center impulse reaches it through the inertia tensor taken in the body's own axes - a box manifold is resolved with one impulse at `supportPoint`, the center of mass clamped into the manifold, not a solve over spin; each contact point then sheds, inertia-weighted, only the spin driving it into its support - angular only, so the unit mismatch stays out of it - and a body resting on something below it has leftover spin under `restAngularSpeed` dropped, so it comes exactly to rest; a body whose center of mass is over a face it rests on, within the degree or so the manifold already counts as flush, is turned flush with the support's face, since nothing torques it the rest of the way down) and `CollisionSystem` (sweep-and-prune in minX order, but responses resolve top-down by bounding-box minY so a stack settles the same whichever side of it the load sits), calling the GJK/EPA narrow phase under `collisions/` — `NarrowPhase.h`'s `findContact`/`intersects` are its entry points. Operates on `ECS3DData` via accessors. OpenMP if available. |
