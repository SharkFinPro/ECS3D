...
**Transport / CLR.** `Protocol.h` defines the format. `Message::write`/`MessageReader::read` take scalars
and enums only, and carry `bool` as an explicit 0/1 byte; an aggregate goes across whole only by
specializing `net::wirePackable`, which is a claim that it has neither padding nor a pointer in it
(`data`'s `WireTypes.h` does this for `glm::vec3` and `uuids::uuid`). `NetServer`/`NetClient` own the
format in C++ and hand `ECS3DNetTransport` (C#) opaque `(type byte, payload)` pairs. `ManagedHost` boots CoreCLR and resolves
