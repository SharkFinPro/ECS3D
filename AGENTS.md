...
| `CMakePresets.json` | The configure/build/test presets: `ecs3d-debug`, `ecs3d-release`, `ecs3d-sanitize`, writing to `cmake-build-debug` / `-release` / `-sanitize`. The build and test presets are hidden (used by name, not listed). `cmake --preset ecs3d-debug` then `cmake --build --preset ecs3d-debug-check` is the documented way in; the raw `--target check` below is what those presets run. |
