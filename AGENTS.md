...
| `source/libs/sim/` | `ECS3DSim` — `PhysicsSystem` (integration, forces, response; linear velocity is displacement per tick while angular velocity is Euler degrees per second, so never add the two in one equation - a box manifold is resolved with one impulse at `supportPoint`, the center of mass clamped into the manifold, not a solve over spin) and `CollisionSystem` (sweep-and-prune), calling the GJK/EPA narrow phase under `collisions/` — `NarrowPhase.h`'s `findContact`/`intersects` are its entry points. Operates on `ECS3DData` via accessors. OpenMP if available. |
