...
| `source/libs/sim/` | `ECS3DSim` — `PhysicsSystem` (integration, forces, response; linear velocity is displacement per tick while angular velocity is a world-space axis in degrees per second, so convert angular velocity to radians per tick before combining the two - `applyForce`, `handleCollision` and `CollisionSystem::fixedUpdate` take `dt` for that; a force is a change of velocity, so the inertia tensor is per unit mass (a solid box whose half-extents are its scale, as `BoxCollider` builds it) and mass enters neither the push nor the spin; it turns the orientation by composing a rotation onto it rather than by adding to the Euler angles, and an off-center impulse reaches it through the inertia tensor taken in the body's own axes - a box manifold is resolved with one impulse at `supportPoint`, the center of mass clamped into the manifold, sized to stop that point's own velocity, spin included, through its effective mass (a dynamic pair trades the closing speed instead), and leaving a body already moving away from its support as it is; before it, each contact point sheds, inertia-weighted, only the spin driving it into its support, and a body resting on something below it has leftover spin under `restAngularSpeed` dropped, so it comes exactly to rest; a body whose center of mass is over a face it rests on, within the degree or so the manifold already counts as flush, is turned flush with the support's face, since nothing torques it the rest of the way down; a box on an edge or corner whose spin would carry its face flat against a box support within the tick, with its center of mass over that face, lands on the face that tick instead, since one tick of edge support turns a box a degree or more and it would otherwise rock from edge to edge) and `CollisionSystem` (sweep-and-prune in minX order, but responses resolve top-down by bounding-box minY so a stack settles the same whichever side of it the load sits), calling the GJK/EPA narrow phase under `collisions/` — `NarrowPhase.h`'s `findContact`/`intersects` are its entry points. Operates on `ECS3DData` via accessors. OpenMP if available. |
