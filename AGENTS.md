...
| `source/libs/sim/` | `ECS3DSim` — `PhysicsSystem` (integration, forces, response; linear velocity is displacement per tick while angular velocity is a world-space axis in degrees per second, so convert angular velocity to radians per tick before combining the two - `applyVelocityChange`, `applyImpulse`, `handleCollision` and `CollisionSystem::fixedUpdate` take `dt` for that; the inertia tensor is per unit mass (a solid box whose half-extents are its scale, as `BoxCollider` builds it), so a velocity change moves and turns a body alike whatever its mass while an impulse is divided by the mass for both; it turns the orientation by composing a rotation onto it rather than by adding to the Euler angles, and an off-center impulse reaches it through the inertia tensor taken in the body's own axes - a box manifold is resolved with one impulse at `supportPoint`, the center of mass clamped into the manifold, sized to stop that point's own velocity, spin included, through its effective mass, and leaving a body already moving away from its support as it is - a dynamic pair sizes the impulse against both bodies' inverse mass and inertia and trades the closing speed elastically (below `minBounceSpeed`, two units a second, it does not bounce), and splits the separation by inverse mass too, but a body that rested on a support last tick hands the downward part of any push from another body to that support, so a stack holds whatever its masses and what lands on it stops as on static geometry, without a bounce - and its support's friction holds the horizontal part too, inside the friction cone and up to what that friction can take (`PhysicsSystem::squeeze`), so a light body is squeezed out from under a heavy one only by the excess, pushed through its center; then Coulomb friction opposes the contact point's sliding up to the coefficient (the geometric mean of two bodies', the body's own against static geometry) times the impulse pressing the pair together, which includes the load stacked on the body (`RigidBody::getStackedLoad`, reset each tick), acting through the center on a face under the center of mass and at the point otherwise, and a resting body's support takes a drag from above first, up to what its own friction holds, leaving the body unturned and recording the grip it spent (`RigidBody::getSpentGrip`, reset each tick) so the body's own contact with that support has only the rest; a face contact also resists turning about its normal, up to friction times load times two thirds of the manifold's mean radius, and a sphere's contact resists rolling at a tenth of that friction, since neither slides at the support point; nothing else damps linear velocity, so a body in flight keeps its horizontal speed; before the impulse, each contact point sheds, inertia-weighted, only the spin driving it into its support, repeated over the points until none sheds more, and a body resting on something below it has leftover spin under `restAngularSpeed` dropped, so it comes exactly to rest; a body whose center of mass is over a face it rests on, within the degree or so the manifold already counts as flush, is turned flush with the support's face, since nothing torques it the rest of the way down; a box on an edge or corner whose spin would carry its face flat against a box support within the tick, with its center of mass over that face, lands on the face that tick instead, since one tick of edge support turns a box a degree or more and it would otherwise rock from edge to edge) and `CollisionSystem` (sweep-and-prune in minX order, but responses resolve top-down by bounding-box minY so a stack settles the same whichever side of it the load sits), calling the GJK/EPA narrow phase under `collisions/` — `NarrowPhase.h`'s `findContact`/`intersects` are its entry points. Operates on `ECS3DData` via accessors. OpenMP if available. |
...
- **Tests** (`source/tests/`) build as `ECS3DTests`, linking `ECS3DData`, `ECS3DSim`, `ECS3DSettings` and
  ...
  `CMakeLists.txt` before adding more. The server's `DefaultProject.cpp` is compiled in the same way. It builds into `<build-dir>/tests`, not `bin/`. GoogleTest is fetched in
...
delete/duplicate (Editor Undo/Redo, below) becomes one atomic edit instead of N separate round trips.
The
...
covering the whole frame) so a body that stalls outright still drops just that connection (applied only when the stream `CanTimeout`;
otherwise the deadline alone bounds it). The handshake, the one
...
`PhysicsSystem`, keeping `scripting` independent of `sim`. Each carries a `ForceMode` (`RigidBody.h`,
mirrored by ScriptBridge's `ForceMode`) that says its units: a force or an impulse is divided by the mass,
...
falling state; `isFalling` is the read-only query scripts get) and `getStackedLoad`/`setStackedLoad`/`getSpentGrip`/`setSpentGrip`
...
group would have left behind, and sends the collected ops as one `replication::buildBatch` - one Ctrl+Z
reverses the whole group in one sceneEdit. A single-command entry is unchanged: no scratch copy, no batch
wrapping, the same payload as before grouping existed.
