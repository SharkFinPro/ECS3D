...
falling state; `isFalling` is the read-only query scripts get) and `getStackedLoad`/`setStackedLoad`
(PhysicsSystem's per-tick record of what rests on the body). `CameraBindings` covers every field on
