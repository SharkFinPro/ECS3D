...
WebSocket on what has actually arrived, since a fragmented message declares none. Neither backend commits
that declared length up front: TCP reads a frame's body in bounded chunks (`TcpBackend.ReadBody`) and
WebSocket grows its assembly buffer as fragments arrive, so a peer that declares a large frame and then
trickles it in a byte at a time pins only what has actually landed, not the whole declared size. TCP backs
that with a per-chunk progress timeout (`BodyReadTimeoutMs`, reset on every chunk rather than covering the
whole frame) so a body that stalls outright still drops just that connection. The handshake, the one
...
- **ECS3DServer** (`apps/server`) — headless authoritative sim. `main.cpp` parses `--project`, `--port`,
  ...
  Ships `defaultAssets/` and generates a built-in `DefaultProject` when no `--project` is given.
